<x-layout>
    ...
    <div class="max-w-3xl mx-auto bg-white rounded-xl shadow p-4">
        ...
        <div class="mt-4 flex items-center gap-3">
            ...
            <div>
                <p class="font-bold">{{ $photo->user->firstname }}</p>
    ...
    <div class="max-w-3xl mx-auto mt-6">
        ...
        @foreach($photo->comments as $comment)
            <div class="bg-white rounded-xl shadow p-3 mb-3 flex items-start gap-3">
                <img src="https://api.dicebear.com/9.x/adventurer/svg?seed={{ $comment->user->firstname }}"
                     class="w-10 h-10 rounded-full">
                <div>
                    <p class="font-bold">{{ $comment->user->firstname }}</p>
                    <p>{{ $comment->comment }}</p>
                </div>
            </div>
        @endforeach
