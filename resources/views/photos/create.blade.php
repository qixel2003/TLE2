<x-layout :heading="'Nieuwe opdracht aanmaken'">
    <form action="{{ route('photos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 p-6 max-w-2xl mx-auto bg-white rounded-2xl shadow-xl">
        ...
        <div class="border-b pb-4 mb-6">
            ...
            <p class="text-gray-600 mt-2">Lever hierin jouw bonus opdracht.</p>
        </div>
        ...
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title') }}"
                   ...
                   required>
            @error('title')
            <div class="text-red-500 mt-1">{{ $photos }}</div>
            @enderror
        </div>
        ...
        <div>
            <label for="image_path" class="block font-semibold text-black mb-2">Ondersteunende foto (optioneel)</label>
        ...
        <div class="pt-6 border-t">
            <div class="flex space-x-4">
                ...
                <a href="{{ route('messages.show') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-3 px-6 rounded-lg transition duration-200">Terug</a>
            </div>
        </div>
    </form>
</x-layout>
