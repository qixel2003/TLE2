    ...
    <form action="{{ route('messages.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="pt-6 border-t">
            <div class="flex space-x-4">
                <button type="submit"
                        class="bg-natuur_groen hover:bg-green-700 text-witte_eend font-semibold py-3 px-6 rounded-lg transition duration-200">
                    Bonus opdracht Aanmaken
                </button>
                <a href="{{ route('messages.index') }}"
                   class="bg-natuur_groen hover:bg-gray-400 text-witte_eend font-semibold py-3 px-6 rounded-lg transition duration-200">Terug</a>
            </div>
        </div>
    </form>
@endsection
